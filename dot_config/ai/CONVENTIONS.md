...
- **Run the sync tool to actually land changes — don't leave it to cron.** System tools (chezmoi, mindmeld) keep machines/indexes in sync; run them as the last step of the work, not passively. After writing KB articles, run `mindmeld sync` — pull, reconcile, push, in one step — and `mindmeld reindex` for the qmd side, since a file on disk that isn't reindexed won't surface in `qmd query`. **`mindmeld sync` does not commit for you**: it refuses rather than sweeping up whatever is sitting there, so commit your own work first and leave anything that isn't yours alone. (Mechanically, since `c803cd0` it refuses on the *collision* — a dirty path the incoming merge would touch — not on any dirt at all; dirty-but-disjoint proceeds with a note. Builds at v0.3.0 and earlier refuse any dirty tree. The instruction is the same either way: commit first.) That refusal is a feature — it is the difference between a tool that lands your changes and one that silently adopts someone else's.
