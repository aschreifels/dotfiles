...
- **Datadog dashboards live in Datadog, not as JSON in the repo.** Build and iterate them in the UI (or push once via the API) and link them from docs/runbooks; don't commit a `*-dashboard.json` mirror — it rots the moment someone tweaks a widget in the UI. Monitors are different: committed definitions + the repo's applier (`apps/curri-workers/monitors/`) are the pattern, because monitor queries are coupled to tag values the code emits and belong in the same commit.
