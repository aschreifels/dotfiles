...
- **`chezmoi add` managed files right after editing them.** When you modify a file chezmoi manages, `chezmoi add` it in the same session so the change reaches other machines via `chezmoi update`. `autoCommit`/`autoPush` are on and pre-authorized — the add commits and pushes, which is the intent for dotfiles. Templatize machine-specific values with chezmoi data (`{{ .chezmoi.homeDir }}` for home paths, the `.machine` var for work/personal blocks) instead of hardcoding a username or home dir — a hardcoded `/Users/<name>` breaks the other machine's `chezmoi update`. **If the auto-push is rejected (remote ahead), run `chezmoi update` (pull + apply) and then re-add** — a bare `chezmoi git pull` merges the *source* but leaves the *target* stale, and the next `chezmoi add` silently overwrites the remote's changes with the stale target (this happened 2026-09-09).
- **Datadog assets live in Datadog, not as JSON in the repo.** Create dashboards and monitors directly in Datadog (UI or API) and link them from docs/runbooks; don't commit `*-dashboard.json` mirrors or monitor definitions by default — they rot the moment someone tweaks a widget or threshold in the UI. The applier-managed set under `apps/curri-workers/monitors/` is a deliberate exception for queries coupled to emitted tag values, not the template; only add to it when I ask.
...
- **Run a gate at its own scope, not at the scope you think the change touched.** A done-criterion that narrows a repo-wide check (`genericize.sh skills docs` instead of bare `genericize.sh`) to the directories you believe you edited defeats the check — a gate exists precisely to catch what you did *not* predict, so scoping it to your prediction is circular. The same failure has a sibling in how specs get written: a task whose brief states a goal and a verification without checking that the verification actually reaches the goal. Worked cases from one session — "open+close a temp file to probe writability" alongside "do not weaken the zero-writes guarantee"; "a rung differs in `Requires` only" alongside a test comparing `Params`; "name the offending field and value" alongside an unstated no-machine-paths rule; and a scoped genericize run while the change landed in `tests/`. Each was implemented faithfully and each shipped a defect, every one caught later by running the *unscoped* gate.
...
- **Memory captures land in the KB, not just Claude's memory store.** The capture bar *is* the KB bar: if a memory is worth writing at all, it's worth a KB entry — there is no lower tier that stays only in Claude's store. The memory interfaces (Claude Code/Desktop) are a cache; necro-kb (git-committed, qmd-queryable) is the source of truth. When you write a memory file, write the corresponding KB entry in the same session (typically `people/`, `patterns/`, or the project's `learnings.md`); the automated sweep is the backstop, not the primary path.
- **`mindmeld` replaces `scribe` entirely — never run `scribe`.** It is retired, and a retired tool fails quietly: `scribe sync` reports `done` and pushes nothing, so KB work sits local while the transcript says it landed. If a stale instruction anywhere still names it, that instruction is the bug.
- **Run the sync tool to actually land changes — don't leave it to cron.** System tools (chezmoi, mindmeld) keep machines/indexes in sync; run them as the last step of the work, not passively. After writing KB articles, run `mindmeld sync` — pull, reconcile, push, in one step — then `qmd update` (BM25 index) and `qmd embed` (vectors) for the search side, since a file on disk that isn't reindexed won't surface in `qmd query`. (`mindmeld reindex` does not exist — mindmeld v0.3.0 has no reindex verb; the qmd CLI owns indexing. Verified 2026-09-17.) **`mindmeld sync` does not commit for you**: it refuses rather than sweeping up whatever is sitting there, so commit your own work first and leave anything that isn't yours alone. (Mechanically, since `c803cd0` it refuses on the *collision* — a dirty path the incoming merge would touch — not on any dirt at all; dirty-but-disjoint proceeds with a note. Builds at v0.3.0 and earlier refuse any dirty tree. The instruction is the same either way: commit first.) That refusal is a feature — it is the difference between a tool that lands your changes and one that silently adopts someone else's.
