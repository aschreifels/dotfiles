...
1. **Delegation threshold.** If the map has fewer than 2 chunks beyond chunk 0, or the whole diff is trivially small, execute inline — orchestration overhead isn't free. `--solo` forces inline. Everything else about the protocol (briefs-as-specs, per-chunk commits, verification) still applies inline; the brief just becomes your own spec.
